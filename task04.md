...
* 从实现方式划分：
  * 基于流水线（pipeline）实现：如下图 所示，基于流水线实现的问答系统有四大核心模块，分别由自然语言理解（NLU）、对话状态跟踪器（DST）、对话策略（DPL）和自然语言生成（NLG）依次串联构成的一条流水线，各模块可独立设计，模块间协作完成任务。
...
<img src="task04.assets/image-20210115221425701.png" alt="image-20210115221425701" style="zoom:50%;" />



* 从答案来源划分：
  * 知识库问答。是目前的研究热点。知识库问答（knowledge base question answering, KB-QA）即给定自然语言问题，通过对问题进行语义理解和解析，进而利用知识库进行查询、推理得出答案。如下图 所示：
  * 常问问题问答；
  * 新闻问答；
  * 网际网路问答；
...
参考：[大话搜索query理解](https://zhuanlan.zhihu.com/p/111904993)，对每一部分做了详细的描述
...
这里应该指的是AC算法（多模式匹配算法），应用的是Trie树。

**AC算法**：

* 多模式匹配算法（与此相对的是单模式匹配算法，如KMP算法，BM算法）

* 核心是寻模式串内部规律，在每次失配时高效跳转。
* 查找模式串的相同前缀关系
* 三张查找表：goto、failure、output，四种具体算法，三张查找表的算法以及本身
...
问题：几年前试过jieba，感觉在专业领域的分词做的不是特别好。有什么能够进一步提升的方法么

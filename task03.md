...
在计算机科学中，图形作为一种特定的数据结构，用于表达数据之间的复杂关系，如社交关系、组织架构、交通信息、网络拓扑等等。
...
它由 Neo Technology支持，从 2003 年开始开发，1.0 版本发布于 2010 年，2.0版本发布于 2013 年。

经过十多年的发展，Neo4j 获得越来越高的关注度，从一个 Java 领域内的图数据库逐渐发展成为适应多语言多框架的图数据库。
...
- 属性。key-value对，存在于**节点**和**关系**中，如图所示。

![图片.png](https://i.loli.net/2020/11/28/OGTbAcSw7UMdzHl.png)
### 2.2 索引

- 动机：与SQL数据库相似，Neo4j使用遍历操作进行查询。为了加速查询，Neo4j会建立索引，并根据索引找到遍历用的起始节点；
...
* Neo4j 支持ACID、集群、备份和故障转移，具有较高的可用性和稳定性；

* 它具备非常好的直观性，通过图形化的界面表示节点和关系；

* 具备较高的可扩展性，能够承载上亿的节点、关系和属性，通过 REST 接口或者面向对象的 JAVA API进行访问。

#### 查询的高性能

Neo4j是一个**原生**的图数据库引擎，它存储了原生的图数据，可以使用图结构的自然伸展特性来设计免索引邻近节点遍历的查询算法，即图的遍历算法设计。

[^1]: 图的遍历是图数据结构所具有的独特算法，即从一个节点开始，根据其连接的关系，可以快速和方便地找出它的邻近节点。这种查找数据的方法并不受数据量的大小所影响，因为邻近查询查找的始终是有限的局部数据，而不会对整个数据库进行搜索。

* Neo4j具有非常高效的查询性能.
* 查询速度不会因数据量的增长而下降，即数据库可以经久耐用，并且始终保持最初的活力。

这里与RDBMS对比

* 相比于RDBMS，它的查询速度可以提高数倍乃至数十倍
...
图数据结构的自然伸展特性及其非结构化的数据格式，让 Neo4j的数据库设计可以具有很大的伸缩性和灵活性。

* 随着需求的变化而增加的节点、关系及其属性并不会影响到原来数据的正常使用
* 没有模式结构定义方面的苦恼

大多数使用关系型数据库的系统，为了应对快速变化的业务需求，往往需要采取推倒重来的方法重构整个应用系统。而这样做的成本是巨大的。使用Neo4j可以最大限度地避免这种情况发生。虽然有时候，也许是因为最初的设计考虑得太不周全，或者为了获得更好的表现力，数据库变更和迁移在所难免，但相比之下更容易

#### 直观明了的数据模型

图数据库设计中的数据模型，从需求的讨论开始，到程序开发和实现，以及最终保存在数据库中的样子，直观明了，似乎没有什么变化，非常适合使用敏捷开发方法。
...
疑问：这里对比的各种图数据库，在以上谈到neo4j的优点时表现如何？用户群体如何？
...
1. 运行neo4j前，需要安装jdk 1.8.0的版本。配置好环境后，在命令行中输入：**java -version**，查看是否安装成功。
2. 测试neo4j是否安装成功，在命令行中输入：**neo4j.bat console**.  (mac可以不配置全局环境变量。切换到文件目录下`bin/neo4j console`运行即可)

`bin/neo4j start`开启服务，在浏览器中使用[http://localhost:7474/browser/](http://localhost:7474/browser/)网址查看数据库（数据库的用户名和密码，一般默认的是：用户：neo4j, 密码：neo4j，但初次登陆会要求更换密码）
...
进入 build_graph.py 类中，加入 本地 Neo4j 图数据库 的 账号和密码；
...
<img src="task03.assets/image-20210113224459213.png" alt="image-20210113224459213" style="zoom:50%;" />
...
这里提到子图的方式批量导入，是否可行，是否会加快速度？（待完成）
...
Neo4j是一个高度可扩展的本机图形数据库，旨在专门利用数据和数据关系。使用Neo4j，开发人员可以构建智能应用程序，以实时遍历当今大型的，相互关联的数据集。大家工作在一个面向对象的、灵活的网络结构下而不是严格、静态的表中，但是可以享受到具备完全的事务特性、企业级的数据库的所有好处。
